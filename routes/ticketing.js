...
router.post('/stock/add', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot add stock to an archived session.' };
  ...
  try {
    await run('INSERT INTO ticket_stock (base_rate_id, color, start_number, end_number, status) VALUES (?, ?, ?, ?, ?)', [base_rate_id, color, parseInt(start_number), parseInt(end_number), 'Available']);
...
router.post('/stock/bulk-upload', isAdmin, upload.single('stockFile'), async (req, res) => {
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot bulk upload to an archived session.' };
  ...
  try {
    ...
    for (const [index, row] of rows.entries()) {
      ...
      await run(
        'INSERT INTO ticket_stock (base_rate_id, color, start_number, end_number, status) VALUES (?, ?, ?, ?, ?)',
        [base_rate_id, color, parseInt(start_number), parseInt(end_number), 'Available']
      );
    }
...
router.post('/stock/edit/:id', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot edit stock in an archived session.' };
    return res.redirect('/ticketing/stock');
  }
...
router.post('/stock/delete/:id', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot delete stock from an archived session.' };
    return res.redirect('/ticketing/stock');
  }
...
router.post('/stock/clear-all', isAdmin, async (req, res) => {
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Can only clear history for the active session.' };
    return res.redirect('/ticketing/stock');
  }
  ...
  try {
    ...
    await run("DELETE FROM accounting_transactions WHERE category = 'Ticket Sales' AND event_session_id = ?", [activeSessionId]);

    // 2. Delete all ticket distributions (both stock-based and imported)
    await run("DELETE FROM ticket_distributions WHERE event_session_id = ?", [activeSessionId]);

    // 3. Delete all ticket stock
    await run("DELETE FROM ticket_stock WHERE event_session_id = ?", [activeSessionId]);

    await run('COMMIT');
    req.session.flash = { type: 'success', message: 'All ticket stock and sales history for the active session has been cleared.' };
  } catch (err) {
    await run('ROLLBACK');
...
router.post('/distribute', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot distribute tickets in an archived session.' };
  ...
  try {
    ...
    const sql = 'INSERT INTO ticket_distributions (distribution_date, staff_id, rate_id, stock_id, distributed_start_number, distributed_end_number) VALUES (date(\'now\'), ?, ?, ?, ?, ?)';
    await run(sql, [staff_id, ride_id, stock_id, stockItem.start_number, stockItem.end_number]);
...
router.post('/distribute/delete/:id', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot recall tickets in an archived session.' };
    return res.redirect('/ticketing/distribute');
  }
...
router.post('/settle/:id', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot settle sales in an archived session.' };
    return res.redirect('/ticketing/settle');
  }
...
router.post('/settle/cancel/:id', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot cancel distributions in an archived session.' };
    return res.redirect('/ticketing/settle');
  }
...
router.post('/unsettle/:id', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot reverse settlements in an archived session.' };
...
router.get('/report/stock', isAdmin, async (req, res) => {
  try {
    ...
    const summaryParams = [];
    if (rate && rate !== 'all') {
      summarySql += ` JOIN base_rates br ON ts.base_rate_id = br.id WHERE br.rate = ?`;
...
router.post('/import', isAdmin, async (req, res) => {
  ...
  if (res.locals.viewingSession.id !== res.locals.activeSession.id) {
    req.session.flash = { type: 'warning', message: 'Cannot import sales to an archived session.' };
  ...
  try {
    ...
    const distSql = `INSERT INTO ticket_distributions (distribution_date, settlement_date, staff_id, rate_id, tickets_sold, calculated_revenue, upi_amount, cash_amount, status, settled_by_user_id, distributed_start_number, distributed_end_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Settled', ?, 0, 0)`;
    const { lastID: distributionId } = await run(distSql, [settlement_date, settlement_date, 1, rate_id, ticketsSold, calculatedRevenue, upiAmount, cashAmount, req.session.user.id]);
